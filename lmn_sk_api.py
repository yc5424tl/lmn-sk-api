...
def gigography_response(artist_id: int):
    ...
    return (jsonify([event.__dict__() for event in event_data]), 200) if event_data else (f'No Gigography For Artist ID {artist_id}', 200)
...
def artist_response(artist_name: str):
    ...
    return (jsonify(artist_data.to_dict()),200) if artist_data \
        else (f'Match not Found For {artist_name}',200)
...
def artist_upcoming_response(artist_id: int):
    ...
    return (jsonify([event.__dict__() for event in upcoming_data]), 200) if upcoming_data \
        else (f'No Upcoming Events Found For {artist_id}', 200)
...
def venue_id_response(venue_id):
    ...
    return (jsonify([venue.__dict__() for venue in venue_data]), 200) if venue_data \
        else (f'No Match Found for {venue_id}', 200)
...
def venue_response(venue_name):
    ...
    return (jsonify([venue.__dict__() for venue in venue_data]), 200) if venue_data \
        else (f'No Match Found for {venue_name}', 200)
...
def venue_upcoming_response(venue_id: int):
    ...
    return (jsonify([event.__dict__() for event in upcoming_data]), 200) if upcoming_data \
        else (f'No Upcoming Events Found for {venue_id}', 200)
...
def metro_area_upcoming(metro_area_id: int):
    ...
    return (jsonify([event.__dict__() for event in upcoming_data]), 200) if upcoming_data \
        else (f'No upcoming events found for {metro_area_id}', 200)
